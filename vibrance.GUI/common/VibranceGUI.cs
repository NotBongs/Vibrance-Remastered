...
namespace vibrance.GUI.common
{
    public partial class VibranceGUI : Form
    {
        ...
        private const string WebsiteLink = "https://bongsy.rip/discord/";
        private const string CashAppLink = "https://cash.app/$Bongsy";
        ...
        private void CheckForUpdates()
        {
            try
            {
                string versionCheckUrl = "https://raw.githubusercontent.com/NotBongs/Vibrance-Remastered/main/Version.txt";
                using (WebClient webClient = new WebClient())
                {
                    ...
                    if (latestVersion > currentVersion)
                    {
                        ...
                        {
                            System.Diagnostics.Process.Start("https://github.com/NotBongs/Vibrance-Remastered/releases");
                        }
                        else
                        {
                            LabelUpdate.Text = "Outdated";
                        }
                    }
                    else
                    {
                        if (latestVersion.ToString() == "0.1.0.1")
                        {
                            LabelUpdate.Text = "BETA";
                        }
                        else
                        {
                            LabelUpdate.Text = latestVersion.ToString();
                        }
                    }
                }
            }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            BetaTimer.Start();
            BetaTimer.Enabled = true;
            notifyIcon = new NotifyIcon();
            notifyIcon.Icon = this.Icon; // Set the icon for the NotifyIcon (you may customize this)
            notifyIcon.Text = "Vibrance GUI"; // Set the tooltip text
            notifyIcon.MouseClick += NotifyIcon_MouseClick; // Attach the event handler
        ...
        private void backgroundWorker_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            if (e.ProgressPercentage == 1)
            {
                ...
                this.statusLabel.ForeColor = Color.MediumOrchid;
            }
        ...
        private void InitializeApplicationList()
        {
            if (this.listApplications.LargeImageList == null)
            {
                ...
                this.listApplications.ForeColor = Color.White;
            }
        }
        ...
        private void labelTwitter_Click(object sender, EventArgs e)
        {
        ...
        private void BetaTimer_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                // Adjust the following ranges to control the color change
                int delta = 25; // Change this to control how much each component can vary

                int currentA = LabelUpdate.ForeColor.A;
                int currentR = LabelUpdate.ForeColor.R;
                int currentG = LabelUpdate.ForeColor.G;
                int currentB = LabelUpdate.ForeColor.B;

                int newA = Clamp(currentA + rand.Next(-delta, delta + 1), 0, 255);
                int newR = Clamp(currentR + rand.Next(-delta, delta + 1), 0, 255);
                int newG = Clamp(currentG + rand.Next(-delta, delta + 1), 0, 255);
                int newB = Clamp(currentB + rand.Next(-delta, delta + 1), 0, 255);

                LabelUpdate.ForeColor = Color.FromArgb(newA, newR, newG, newB);
            }
        }
